...
SSIS Package can be scheduled using SQL Agent. There is no default agent deployed for the package and it is up to the user to create one. The package does delta loads of logger table and full loads of meta tables. Since meta tables are relatively small, the more often the package runs, the less logger data it collects which every run.
